...
import {
  ...
  type EstimatorSensitivityReport,
...
const reportPath = resolve(
  option("report") ??
    "simulation/results/estimator-sensitivity-exploratory-v1.json"
);
const planPath = resolve(
  option("plan") ??
    "simulation/plans/estimator-sensitivity-exploratory-v1.json"
);
const bankPath = resolve(option("bank") ?? "public/jacet_parameters.csv");
const report = JSON.parse(
  readFileSync(reportPath, "utf8")
) as EstimatorSensitivityReport;
const planBytes = readFileSync(planPath);
const bankBytes = readFileSync(bankPath);
const plan = JSON.parse(planBytes.toString("utf8")) as EstimatorSensitivityPlan;

assert.equal(report.schemaVersion, "estimator-sensitivity-v1");
...
const recomputed = runEstimatorSensitivity(
