...
import {
  ...
  type ExposureSimulationProvenance,
...
const sourceSha256 = Object.fromEntries(
  [
    "scripts/simulate-exposure-control.ts",
    "src/utils/exposureSimulation.ts",
    "src/utils/sympsonHetter.ts",
    "src/utils/abilityEstimators.ts",
    "src/utils/paperScoring.ts",
  ].map((relativePath) => [relativePath, sha256(readFileSync(relativePath))])
);
const provenance: ExposureSimulationProvenance = {
  nodeVersion: process.version,
  platform: process.platform,
  architecture: process.arch,
  sourceSha256,
};
const report = runExposureSimulation(
